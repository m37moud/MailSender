...
Tel: ${envDefaults.senderPhone}`,
  ...
  async _showDebugInfo() {
    try {
      ...
      const debugInfo = {
        'Extension Version': Environment ? Environment.getVersion() : 'Environment not loaded',
        'Client ID': Environment ? (Environment.getClientId() ? 'Present' : 'Missing') : 'Environment not loaded',
        'EmailService': this.emailService ? 'Initialized' : 'Not initialized',
        'StorageService': typeof StorageService !== 'undefined' ? 'Available' : 'Not available'
      };

      // Try to get more detailed info
      try {
        const config = await StorageService.getEmailConfig();
        debugInfo['Configuration'] = config ? 'Present' : 'Missing';
        debugInfo['Has Attachment'] = config && config.attachmentData ? 'Yes' : 'No';
        
        if (config && config.attachmentData) {
          const sizeKB = Math.round(config.attachmentData.length * 0.75 / 1024);
          debugInfo['Attachment Size'] = `${sizeKB} KB`;
        }
      } catch (storageError) {
        debugInfo['Storage Error'] = storageError.message;
      }
      ...
      try {
        if (this.emailService) {
          const authStatus = await this.emailService.getAuthStatus();
          debugInfo['Authentication'] = authStatus.isAuthenticated ? 'Authenticated' : 'Not authenticated';
        }
      } catch (authError) {
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    console.log('Environment available:', typeof Environment);
